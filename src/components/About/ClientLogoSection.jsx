...
// The 'clients' array and 'ClientLogo' component remain the same.
// const clients = [
//   { name: "Epihfermal", logo: "/About/logos/ephiermal1.png" },
//   { name: "Bluiera", logo: "/About/logos/bluiera.png" },
//   { name: "Vkopa", logo: "/About/logos/vkopa.png" },
...
const ClientLogo = ({ name, logo }) => (
  // We remove padding/margins from here to let the grid gaps handle spacing
  // The centering within the grid cell will be handled by justify-items-center
  <div className="client-logo-item flex items-center gap-2 py-2 hover:-translate-y-1 transition duration-200 cursor-pointer">
    <div className="w-8 h-8 relative">
      <Image
        src={logo}
        alt={name}
  ...
  return (
    <section className="py-16 text-center max-w-[1520px]">
      <div className="max-w-[1520px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 max-w-[1480px] mx-auto">
          {/* Using CSS Grid for a more consistent row width */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-3 gap-y-2 sm:gap-x-5 sm:gap-y-3 lg:gap-x-8 lg:gap-y-4 justify-items-center">
            {clients.map((client, idx) => (
              <ClientLogo key={idx} {...client} />
            ))}
          </div>
